...
META_FORMAT = "<HHIQQQ"  # version, page_size, segment_count, total_pages, tail, created_at
VERSION = 1
...
class RelationFile:
    ...
    def __write_metadata(self):
        with open(os.path.join(self._path, "metadata.pydb"), "r+b") as f:
            data = struct.pack(META_FORMAT, VERSION, PAGE_SIZE, self.__total_number_of_data_files(), 0, 0, int(time.time()))
            f.write(data)
            f.flush()
            os.fsync(f.fileno())
